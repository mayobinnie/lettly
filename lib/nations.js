  ...
  const items = {
    before_let: {
      ...
      items: [
        ...
        { id: 'co_alarm',      label: 'Fit a CO alarm in every room with any fixed combustion appliance (gas boiler, oil boiler, log burner, solid fuel) and in any sleeping room with a solid fuel appliance (from Oct 2022)', required: true },
        { id: 'consent_let',   label: 'Get consent to let from your mortgage lender (if mortgaged)', required: false },
        { id: 'hmo_check',     label: 'Check if your property needs an HMO licence - required if 5 or more people from 2 or more households share facilities. Some councils require licences for smaller HMOs. Check with your local council.', required: false },
        { id: 'selective_lic', label: 'Check if your area has selective or additional licensing - some councils require ALL private landlords to hold a licence regardless of property type. Check your local council website.', required: false },
        ...(nation === 'Scotland' ? [
    ...
    financial: {
      ...
      items: [
        ...
        { id: 'tenant_fees',   label: 'Understand the Tenant Fees Act 2019 - you cannot charge tenants for referencing, admin, credit checks, viewings or application fees. Only permitted: rent, deposit (max 5 weeks), holding deposit (max 1 week), late payment interest, key replacement, and tenant-requested changes to tenancy.', required: true },
    ...
    tenancy_setup: {
      ...
      items: [
        ...
        { id: 'tenancy_agmt',  label: nation === 'Wales' ? 'Use a Written Statement / Occupation Contract (required under Renting Homes Wales Act) - must be provided within 14 days of tenancy start. Use the Welsh Government model as a base.' : nation === 'Scotland' ? 'Provide written Private Residential Tenancy (PRT) agreement using the Scottish Government model tenancy or equivalent, plus the Tenancy Information Pack and Easy Read Notes' : 'Provide a written tenancy agreement and the current How to Rent guide (always use the most recent version - check gov.uk/how-to-rent)', required: true },
        { id: 'holding_dep',  label: 'If taking a holding deposit to reserve the property: maximum is 1 week\'s rent. Must be returned within 15 days unless tenant withdraws, fails referencing, or provides false information. It is deducted from the first month\'s rent if the tenancy proceeds.', required: false },
        { id: 'deposit_prot',  label: 'Protect the tenancy deposit in a government-approved scheme within 30 days of receipt', required: true },
        { id: 'prescribed',    label: 'Serve Prescribed Information about the deposit scheme on the tenant within 30 days - must include scheme details, landlord and tenant names and addresses, the deposit amount, property address, the scheme leaflet, and any applicable terms. Tenant must sign or acknowledge receipt.', required: true },
        { id: 'give_gas',      label: 'Give tenant a copy of the Gas Safety Certificate', required: true },
        { id: 'give_epc',      label: 'Give tenant a copy of the EPC (EPC must also be available to prospective tenants during marketing of the property - you cannot market without a valid EPC)', required: true },
        { id: 'give_eicr',     label: 'Give tenant a copy of the EICR', required: true },
        { id: 'inventory',     label: 'Carry out a detailed inventory and schedule of condition with the tenant - not legally required but essential for any deposit dispute. Photograph everything and get tenant signature.', required: false },
        ...(nation === 'England' ? [
    ...
    ongoing: {
      ...
      icon: '🔄',
      items: [
        { id: 'annual_gas',    label: 'Renew Gas Safety Certificate every year (book 4-6 weeks before expiry). You must give the renewed certificate to existing tenants within 28 days of the check being carried out.', required: true },
        ...
        ...(nation === 'England' ? [
          { id: 'fitness_hab', label: 'Homes (Fitness for Human Habitation) Act 2018: property must be fit for human habitation throughout the tenancy. Covers 29 hazard categories including damp, structural safety, heating, electrical hazards, pests and sanitation.', required: true },
          ...
          { id: 'smoke_ongoing', label: 'Ensure smoke and CO alarms remain in good repair throughout the tenancy - not just at the start', required: true },
        ] : []),
      ]
    }
  }
