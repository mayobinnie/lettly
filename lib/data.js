...
export function mergeDoc(portfolio, extracted) {
  ...
  function matchScore(prop) {
    ...
    const extractedPC = (normExtracted.match(postcodeRe) || [''])[0].replace(/\s/g,'').toLowerCase()
    const propPC = (normAddr.match(postcodeRe) || [''])[0].replace(/\s/g,'').toLowerCase()

    // Exact postcode match = definitive
    if (extractedPC && propPC && extractedPC === propPC) return 100
  ...
  if (c.insurance) {
    ...
    if (ins.premium)              patch.insurancePremium     = ins.premium
    if (ins.sumInsured)           patch.insuranceSumInsured  = ins.sumInsured
    if (ins.buildingsSum)         patch.insuranceBuildings   = ins.buildingsSum
    if (ins.liabilitySum)         patch.insuranceLiability   = ins.liabilitySum
    if (ins.excess)               patch.insuranceExcess      = ins.excess
    if (ins.lossOfRentCover)      patch.insuranceLossOfRent  = ins.lossOfRentCover
    ...
    if (ins.legalExpensesCover)   patch.insuranceLegal       = ins.legalExpensesCover
  ...
  if (t.tenantName) {
    // Safety check: don't overwrite correct tenant with landlord name
    // If the extracted name matches the portfolio owner name, it's likely a mis-extraction
