...
export function mergeDoc(portfolio, extracted) {
  ...
  // Score-based matching - avoids false positives from partial street name matches
  function matchScore(prop) {
    const normAddr = normalise(prop.address || '')
    const normPropShort = normalise(prop.shortName || '')
    
    // Extract postcode from both (most reliable identifier)
    const postcodeRe = /[a-z]{1,2}[0-9][0-9a-z]?\s*[0-9][a-z]{2}/i
    const extractedPC = (normExtracted.match(postcodeRe) || [''])[0].replace(/\s/g,'')
    const propPC = (normAddr.match(postcodeRe) || [''])[0].replace(/\s/g,'')
    
    if (extractedPC && propPC && extractedPC === propPC) return 100 // exact postcode match
    if (normAddr === normExtracted) return 90 // exact address match
    if (normPropShort && normExtracted.includes(normPropShort)) return 70
    if (normExtracted && normAddr.includes(normExtracted.slice(0,20))) return 60
    return 0
  }

  const scores = props.map((p, i) => ({ i, score: matchScore(p) }))
  const best = scores.reduce((a, b) => b.score > a.score ? b : a, { i: -1, score: 0 })
  const matchIdx = best.score >= 60 ? best.i : -1
