...
export function mergeDoc(portfolio, extracted) {
  ...
  function matchScore(prop) {
    ...
    // House number must match if both have one - prevents 11 vs 3 confusion
    ...
    if (extractedNum && propNum && extractedNum !== propNum) return 0 // different number = no match

    // Short name contained in extracted address (with number check passed)
    ...
    // Substantial address overlap (first 25 chars after number)
    const extractedCore = normExtracted.replace(/^\d+[a-z]?\s+/i,'').slice(0,25)
    const propCore = normAddr.replace(/^\d+[a-z]?\s+/i,'').slice(0,25)
    if (extractedCore.length > 10 && propCore.length > 10 && extractedCore === propCore) return 65
  ...
  // Raised threshold to 65 - must be a confident match
  const matchIdx = best.score >= 65 ? best.i : -1
