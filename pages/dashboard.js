...
function Metric({ label, value, sub, subGreen }) {
  return (
    <div style={{ background:'var(--surface2)', borderRadius:'var(--radius)', padding:'14px 16px' }}>
      ...
      <div style={{ fontSize:22, fontWeight:600, fontFamily:'var(--mono)', letterSpacing:'-0.5px' }}>{value}</div>
...
function DropZone({ onFiles, compact }) {
  ...
  if (compact) {
    return (
      <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={handleDrop} onClick={()=>inputRef.current.click()}
        style={{ border:`1.5px dashed ${over?'var(--brand)':'var(--border-strong)'}`, borderRadius:12, padding:'16px 20px', cursor:'pointer', background:over?'var(--brand-subtle)':'transparent', transition:'all 0.2s', display:'flex', alignItems:'center', gap:12 }}>
  ...
  return (
    <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={handleDrop} onClick={()=>inputRef.current.click()}
      style={{ border:`2px dashed ${over?'var(--brand)':'rgba(0,0,0,0.14)'}`, borderRadius:24, padding:'56px 40px 48px', textAlign:'center', background:over?'var(--brand-subtle)':'var(--surface)', cursor:'pointer', transition:'all 0.25s ease', boxShadow:over?'0 0 0 6px rgba(27,94,59,0.06)':'var(--shadow-sm)' }}>
      <input ref={inputRef} type="file" multiple accept=".pdf,image/*" style={{display:'none'}} onChange={e=>{onFiles(Array.from(e.target.files));e.target.value=''}} />
      <div className={over?'':'floating'} style={{ width:76, height:76, borderRadius:'50%', margin:'0 auto 22px', background:over?'var(--brand)':'var(--brand-light)', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.25s' }}>
        <UploadIcon color={over?'#fff':'var(--brand)'} size={32} />
      </div>
      <div style={{ fontFamily:'var(--display)', fontSize:26, fontWeight:400, color:'var(--text)', marginBottom:10, lineHeight:1.2 }}>
        {over ? 'Release to analyse' : 'Drop your documents here'}
      </div>
      <div style={{ fontSize:14, color:'var(--text-2)', lineHeight:1.75, marginBottom:28 }}>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:8, justifyContent:'center', marginBottom:24 }}>
        {Object.values(DOC_META).filter(d=>d.label!=='Document').map(d => (
          <span key={d.label} style={{ fontSize:11, padding:'5px 13px', borderRadius:20, background:d.bg, color:d.fg, display:'inline-flex', alignItems:'center', gap:5 }}>
            <span style={{fontSize:13}}>{d.icon}</span>{d.label}
          </span>
        ))}
      </div>
      <div style={{ fontSize:12, color:'var(--text-3)' }}>PDF or image files · your data stays private</div>
    </div>
  )
}
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {isDone && ext && (
          <div style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.6 }}>
            {ext.summary}
            {ext.property?.shortName && <span style={{ marginLeft:8, color:'var(--brand)', fontWeight:500 }}>· {ext.property.shortName}</span>}
          </div>
        )}
        {isDone && ext?.documentType && <div style={{ marginTop:7 }}><DocBadge type={ext.documentType}/></div>}
        {isError && <div style={{ fontSize:11, color:'var(--red)', marginTop:4 }}>Could not read this file. Make sure it is a clear PDF or image.</div>}
      </div>
    </div>
  )
}
    ...
    <div className="fade-up">
      {urgent.length > 0 && (
        <div style={{ background:'#fce8e6', border:'0.5px solid #E24B4A', borderRadius:12, padding:'13px 16px', marginBottom:16, color:'#791F1F', fontSize:12, lineHeight:1.8 }}>
          <div style={{ fontWeight:600, marginBottom:5 }}>⚠️ {urgent.length} urgent action{urgent.length>1?'s':''}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
        ...
        <Metric label="Actions needed" value={urgent.length+upcoming.length} sub={urgent.length>0?`${urgent.length} urgent now`:'Nothing urgent'} />
      </div>
      {props.length === 0 ? (
        <div style={{ textAlign:'center', padding:'48px 24px', background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:16 }}>
          <div style={{ fontFamily:'var(--display)', fontSize:22, fontWeight:300, color:'var(--text-2)', marginBottom:10 }}>No properties yet</div>
          <div style={{ fontSize:13, color:'var(--text-3)', marginBottom:22 }}>Drop your property documents above to get started.</div>
        ...
        <div style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:16, padding:18 }}>
          <div style={{ fontSize:13, fontWeight:500, marginBottom:14 }}>Upcoming compliance</div>
          {upcoming.length === 0 && urgent.length === 0
            ? <div style={{ fontSize:12, color:'var(--text-3)', padding:'8px 0' }}>No upcoming issues in your documents.</div>
            : upcoming.map((t,i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:i<upcoming.length-1?'0.5px solid var(--border)':'none' }}>
      ...
      <div style={{ background:'var(--brand-subtle)', border:'0.5px solid rgba(27,94,59,0.15)', borderRadius:12, padding:'14px 16px', marginTop:14 }}>
        <div style={{ fontSize:12, fontWeight:600, color:'var(--brand)', marginBottom:5 }}>Renters&#39; Rights Bill — Oct 2026</div>
        <div style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.7 }}>Section 21 no-fault evictions will be abolished. All tenancies become periodic. PRS Database registration required before serving any notice. Ask Lettly AI for a personalised impact assessment.</div>
      </div>
    </div>
  )
}
...
function Properties({ portfolio, onAddDocs }) {
  ...
  if (props.length===0) return (
    <div className="fade-up" style={{ textAlign:'center', padding:'48px 24px', background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:16 }}>
      <div style={{ fontFamily:'var(--display)', fontSize:22, fontWeight:300, color:'var(--text-2)', marginBottom:10 }}>No properties yet</div>
      <div style={{ fontSize:13, color:'var(--text-3)', marginBottom:22 }}>Drop your documents to add properties automatically.</div>
      <button onClick={onAddDocs} style={{ background:'var(--brand)', color:'#fff', border:'none', borderRadius:8, padding:'10px 22px', fontSize:13, fontWeight:500, cursor:'pointer' }}>+ Drop documents</button>
    </div>
  )
  return (
    <div className="fade-up">
      {props.map(p => {
        const gasC  = dueSoon(p.gasDue)
        const eicrC = dueSoon(p.eicrDue)
        const insC  = dueSoon(p.insuranceRenewal)
        ...
        return (
          <div key={p.id} style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:16, padding:20, marginBottom:14, boxShadow:'var(--shadow-sm)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
              <div>
                <div style={{ fontFamily:'var(--display)', fontSize:22, fontWeight:400, color:'var(--text)', marginBottom:3 }}>{p.shortName}</div>
              ...
              {p.rent && (
                <div style={{ textAlign:'right', flexShrink:0, marginLeft:16 }}>
                  <div style={{ fontSize:24, fontWeight:600, fontFamily:'var(--mono)', letterSpacing:'-0.5px' }}>{fmt(p.rent)}</div>
...
function AITab({ portfolio }) {
  ...
  const [messages, setMessages] = useState([{
    role:'assistant',
    content: n>0
      ? `I can see your portfolio of ${n} propert${n===1?'y':'ies'}. Ask me anything about compliance, the Renters' Rights Bill, your finances, or what to prioritise.`
  ...
  const quickQ = n>0 ? [
    ...
    'Build me a 6-month action plan',
  ...
  return (
    <div className="fade-up">
      <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:12 }}>
        {quickQ.map((q,i) => (
          <button key={i} onClick={()=>send(q)} style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:20, padding:'5px 13px', fontSize:11, color:'var(--text-2)', cursor:'pointer' }}
            onMouseEnter={e=>e.target.style.borderColor='var(--brand-mid)'} onMouseLeave={e=>e.target.style.borderColor='var(--border)'}>{q}</button>
        ))}
      </div>
      <div style={{ display:'flex', flexDirection:'column', height:500 }}>
        <div ref={scrollRef} style={{ flex:1, overflowY:'auto', padding:14, display:'flex', flexDirection:'column', gap:12, background:'var(--surface2)', borderRadius:'14px 14px 0 0', border:'0.5px solid var(--border)', borderBottom:'none' }}>
          {messages.map((m,i) => (
            <div key={i} style={{ maxWidth:'88%', alignSelf:m.role==='user'?'flex-end':'flex-start' }}>
              {m.role==='assistant' && <div style={{ fontSize:10, color:'var(--brand)', marginBottom:4, textTransform:'uppercase', letterSpacing:'0.6px', fontWeight:600 }}>Lettly AI</div>}
              <div style={{ padding:'10px 14px', borderRadius:m.role==='user'?'12px 12px 2px 12px':'12px 12px 12px 2px', background:m.role==='user'?'var(--brand)':'var(--surface)', color:m.role==='user'?'#fff':'var(--text)', border:m.role==='assistant'?'0.5px solid var(--border)':'none', fontSize:12, lineHeight:1.7, whiteSpace:'pre-wrap' }}>{m.content}</div>
            </div>
          ))}
          {loading && (
        ...
        <div style={{ display:'flex', gap:8, background:'var(--surface)', border:'0.5px solid var(--border)', borderTop:'none', borderRadius:'0 0 14px 14px', padding:10 }}>
          <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} placeholder="Ask about compliance, legislation or your portfolio…" rows={1}
            style={{ flex:1, background:'var(--surface2)', border:'0.5px solid var(--border)', borderRadius:8, padding:'8px 12px', fontFamily:'var(--font)', fontSize:12, color:'var(--text)', resize:'none', outline:'none' }} />
          <button onClick={()=>send()} disabled={loading||!input.trim()} style={{ background:'var(--brand)', color:'#fff', border:'none', borderRadius:8, padding:'8px 20px', fontSize:12, fontWeight:500, cursor:(loading||!input.trim())?'not-allowed':'pointer', opacity:(loading||!input.trim())?0.5:1 }}>Send</button>
        </div>
      </div>
    </div>
  )
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!storageKey) return
    try { localStorage.setItem(storageKey, JSON.stringify(portfolio)) } catch {}
  ...
  if (!isLoaded || !isSignedIn) return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ width:32, height:32, borderRadius:'50%', border:'2.5px solid var(--brand)', borderTopColor:'transparent', animation:'spin 0.75s linear infinite' }}/>
    </div>
  )

  const propCount = portfolio.properties?.length || 0
      ...
      <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
        ...
        <nav style={{ background:'var(--surface)', borderBottom:'0.5px solid var(--border)', padding:'0 24px', display:'flex', alignItems:'center', justifyContent:'space-between', height:58, position:'sticky', top:0, zIndex:100 }}>
          ...
          <div style={{ display:'flex', gap:1, background:'var(--surface2)', padding:3, borderRadius:10 }}>
            {TABS.map(t => (
              <button key={t.id} onClick={()=>setTab(t.id)} style={{ background:tab===t.id?'var(--surface)':'transparent', border:tab===t.id?'0.5px solid var(--border)':'none', padding:'5px 16px', borderRadius:7, fontFamily:'var(--font)', fontSize:12, color:tab===t.id?'var(--text)':'var(--text-2)', fontWeight:tab===t.id?500:400, cursor:'pointer', boxShadow:tab===t.id?'var(--shadow-sm)':'none' }}>
                {t.label}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <button onClick={()=>setShowDrop(v=>!v)} style={{ background:'none', border:'0.5px solid var(--border-strong)', borderRadius:8, padding:'6px 14px', fontSize:12, color:'var(--text-2)', cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
        ...
        {/* Queue */}
        {queue.length > 0 && (
          <div style={{ background:'var(--surface)', borderBottom:'0.5px solid var(--border)', padding:'12px 24px' }}>
        ...
        <div style={{ maxWidth:1060, margin:'0 auto', padding:'28px 20px' }}>
          ...
          {tab==='overview' && (
            <div style={{ marginBottom:20 }}>
              <h1 style={{ fontFamily:'var(--display)', fontSize:28, fontWeight:300, color:'var(--text)', marginBottom:4 }}>
                Good {getGreeting()}, {firstName}
              </h1>
              <p style={{ fontSize:13, color:'var(--text-3)' }}>
                {propCount===0
                  ? 'Drop your documents below to build your portfolio.'
                  : `${propCount} propert${propCount===1?'y':'ies'} in your portfolio.`}
              </p>
            </div>
          )}

          {tab==='overview'   && <Overview   portfolio={portfolio} onAddDocs={()=>setShowDrop(true)} />}
