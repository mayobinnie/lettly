...
if(compact){const borderCol=over?'var(--brand)':'var(--border-strong)';return<div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={drop} onClick={()=>ref.current.click()} style={{border:'1.5px dashed '+borderCol,borderRadius:12,padding:'12px 16px',cursor:'pointer',background:over?'var(--brand-subtle)':'var(--surface)',display:'flex',alignItems:'center',gap:12,transition:'all 0.15s'}}><input ref={ref} type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.heic,.heif,.bmp,.tiff,.gif,image/*,application/pdf" style={{display:'none'}} onChange={e=>{onFiles(Array.from(e.target.files));e.target.value=''}}/>
  <div style={{width:30,height:30,borderRadius:8,background:over?'var(--brand)':'var(--brand-light)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><UpIcon color={over?'#fff':'var(--brand)'} size={14}/></div>
  <div style={{flex:1}}><div style={{fontSize:12,fontWeight:500,color:'var(--text)'}}>Drop or click to add documents</div><div style={{fontSize:11,color:'var(--text-3)'}}>PDF, JPEG, HEIC — gas cert, EICR, EPC, insurance, tenancy</div></div>
  <div style={{fontSize:11,color:'var(--brand)',fontWeight:500,flexShrink:0,whiteSpace:'nowrap'}}>{over?'Release to upload':'Browse'}</div></div>}</div>
