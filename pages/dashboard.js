...
function RentabilityChecklist({prop}){
  ...
  const checks = [
    ...
    {id:'buildings_ins', label:'Buildings insurance', status:(()=>{const t=(prop.insuranceType||'').toLowerCase();if(!prop.insurer)return 'missing';if(t==='home')return 'fail';if(t.includes('landlord')||t.includes('buildings')||t.includes('building')||t==='')return 'done';return 'done'})(), hint:'Buildings cover required if you own the freehold - not needed for leasehold flats where freeholder insures'},
    {id:'ins',    label:'Landlord liability insurance', status:(()=>{const t=(prop.insuranceType||'').toLowerCase();if(!prop.insurer)return 'missing';if(t==='home')return 'fail';return 'done'})(), hint:'Public liability and landlord policy - must not be a standard home insurance policy'},
...
export default function Dashboard(){
  ...
  useEffect(()=>{
    ...
    if(!wizardDone){
      ...
      // Draft count badge loaded when content queue tab is opened
