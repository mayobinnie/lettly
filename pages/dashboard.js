...
if(compact)return<div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={drop} onClick={()=>ref.current.click()} style={{border:`1.5px dashed ${over?'var(--brand)':'var(--border-strong)'}`,borderRadius:12,padding:'12px 16px',cursor:'pointer',background:over?'var(--brand-subtle)':'var(--surface)',display:'flex',alignItems:'center',gap:12,transition:'all 0.15s'}}><input ref={ref} type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.heic,.heif,.bmp,.tiff,.gif,image/*,application/pdf" style={{display:'none'}} onChange={e=>{onFiles(Array.from(e.target.files));e.target.value=''}}/>
