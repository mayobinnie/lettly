...
function Pill({ type='grey', dot, children }) {
  ...
  return <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:11, fontWeight:500, padding:'3px 10px', borderRadius:20, background:c.bg, color:c.fg, whiteSpace:'nowrap' }}>
    {dot&&<span style={{ width:5, height:5, borderRadius:'50%', background:c.fg, flexShrink:0 }}/>}{children}
  </span>
}
...
function Metric({ label, value, sub, subGreen, subRed }) {
  return <div style={{ background:'var(--surface2)', borderRadius:'var(--radius)', padding:'14px 16px' }}>
...
function SectionHead({ title, action, onAction }) {
  return <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
    ...
    {action&&<button onClick={onAction} style={{ fontSize:11, color:'var(--brand)', background:'none', border:'none', cursor:'pointer', fontWeight:500 }}>{action}</button>}
  </div>
}
...
function DocBadge({ type }) {
  ...
  return <span style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:11, fontWeight:500, padding:'3px 10px', borderRadius:20, background:m.bg, color:m.fg }}>
    <span style={{fontSize:12}}>{m.icon}</span>{m.label}
  </span>
}

/* ─── DropZone ─────────────────────────────────────────── */
function UpIcon({ color, size }) {
...
function DropZone({ onFiles, compact }) {
  ...
  function drop(e){e.preventDefault();setOver(false);const f=Array.from(e.dataTransfer.files).filter(f=>f.type==='application/pdf'||f.type.startsWith('image/'));if(f.length)onFiles(f)}
  if(compact) return <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={drop} onClick={()=>ref.current.click()}
    style={{ border:`1.5px dashed ${over?'var(--brand)':'var(--border-strong)'}`, borderRadius:12, padding:'14px 16px', cursor:'pointer', background:over?'var(--brand-subtle)':'transparent', display:'flex', alignItems:'center', gap:12 }}>
    <input ref={ref} type="file" multiple accept=".pdf,image/*" style={{display:'none'}} onChange={e=>{onFiles(Array.from(e.target.files));e.target.value=''}}/>
    <div style={{ width:32, height:32, borderRadius:8, background:over?'var(--brand)':'var(--brand-light)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><UpIcon color={over?'#fff':'var(--brand)'} size={15}/></div>
    <div><div style={{ fontSize:12, fontWeight:500 }}>Drop documents</div><div style={{ fontSize:11, color:'var(--text-3)' }}>Gas cert, EICR, EPC, insurance, tenancy</div></div>
  </div>
  return <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setOver(false)}} onDrop={drop} onClick={()=>ref.current.click()}
    ...
    <input ref={ref} type="file" multiple accept=".pdf,image/*" style={{display:'none'}} onChange={e=>{onFiles(Array.from(e.target.files));e.target.value=''}}/>
    <div className={over?'':'floating'} style={{ width:64, height:64, borderRadius:'50%', margin:'0 auto 16px', background:over?'var(--brand)':'var(--brand-light)', display:'flex', alignItems:'center', justifyContent:'center' }}><UpIcon color={over?'#fff':'var(--brand)'} size={28}/></div>
    ...
    <div style={{ fontSize:13, color:'var(--text-2)', lineHeight:1.75, marginBottom:18 }}>Gas certs · EICRs · Insurance · EPCs · Tenancy agreements · Mortgage offers</div>
    <div style={{ display:'flex', flexWrap:'wrap', gap:6, justifyContent:'center', marginBottom:14 }}>
      {Object.values(DOC_META).filter(d=>d.label!=='Document').map(d=><span key={d.label} style={{ fontSize:11, padding:'4px 11px', borderRadius:20, background:d.bg, color:d.fg, display:'inline-flex', alignItems:'center', gap:4 }}><span style={{fontSize:12}}>{d.icon}</span>{d.label}</span>)}
    </div>
...
function QueueItem({ item }) {
  const done=item.status==='done', err=item.status==='error', working=item.status==='reading'||item.status==='extracting'
  const ext=item.result?.extracted
  return <div className="scale-in" style={{ display:'flex', gap:12, alignItems:'flex-start', background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:12, padding:'12px 14px' }}>
    <div style={{ width:38, height:38, borderRadius:9, flexShrink:0, background:done?'var(--brand-light)':err?'var(--red-bg)':'var(--surface2)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      {done&&<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--brand)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>}
      {err&&<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="12" r="9"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>}
      {working&&<div style={{ width:18, height:18, borderRadius:'50%', border:'2px solid var(--brand)', borderTopColor:'transparent', animation:'spin 0.75s linear infinite' }}/>}
    </div>
    <div style={{ flex:1, minWidth:0 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:done&&ext?5:0 }}>
        <div style={{ fontSize:12, fontWeight:500, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'60%' }}>{item.name}</div>
        <Pill type={done?'green':err?'red':item.status==='extracting'?'amber':'grey'}>{done?'Extracted ✓':err?'Error':item.status==='extracting'?'Analysing…':'Reading…'}</Pill>
      </div>
      {done&&ext&&<div style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.6 }}>{ext.summary}{ext.property?.shortName&&<span style={{ marginLeft:6, color:'var(--brand)', fontWeight:500 }}>· {ext.property.shortName}</span>}</div>}
      {done&&ext?.documentType&&<div style={{ marginTop:6 }}><DocBadge type={ext.documentType}/></div>}
      {err&&<div style={{ fontSize:11, color:'var(--red)', marginTop:3 }}>{item.result?.error||'Could not read this file.'}</div>}
    </div>
  </div>
}
...
const EMPTY_PROP = {
  shortName:'', address:'', ownership:'Personal',
  purchasePrice:'', currentValue:'', mortgage:'', lender:'', rate:'', fixedEnd:'', monthlyPayment:'', ercRate:'',
  rent:'', tenantName:'', tenantPhone:'', tenantEmail:'', tenancyStart:'', tenancyEnd:'', depositAmount:'', depositScheme:'',
  gasDue:'', eicrDue:'', epcRating:'', epcExpiry:'', insurer:'', policyNo:'', insuranceRenewal:'', insuranceType:'',
  notes:''
}
...
function Select({ label, value, onChange, options }) {
  return <div style={{ marginBottom:14 }}>
    ...
    <select value={value||''} onChange={e=>onChange(e.target.value)}
      style={{ width:'100%', background:'var(--surface2)', border:'0.5px solid var(--border-strong)', borderRadius:8, padding:'8px 11px', fontFamily:'var(--font)', fontSize:13, color:'var(--text)', outline:'none' }}>
...
function PropertyForm({ initial, onSave, onDelete, onClose }) {
  ...
  const tabs = ['basics','finance','tenant','compliance']

  function handleSave() {
    if (!p.shortName.trim()) return alert('Please enter a property name')
    onSave({ ...p, id: p.id || Math.random().toString(36).slice(2), docs: p.docs || [] })
    onClose()
  }
    ...
    <div style={{ background:'var(--surface)', borderRadius:'20px 20px 0 0', width:'100%', maxWidth:680, maxHeight:'92vh', display:'flex', flexDirection:'column', boxShadow:'0 -8px 40px rgba(0,0,0,0.12)' }}>
      ...
      <div style={{ padding:'18px 20px 0', flexShrink:0 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          ...
          <button onClick={onClose} style={{ background:'var(--surface2)', border:'none', borderRadius:'50%', width:30, height:30, cursor:'pointer', fontSize:16, color:'var(--text-2)', display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
        </div>
        <div style={{ display:'flex', gap:4, marginBottom:0 }}>
          {tabs.map(t=><button key={t} onClick={()=>setTab(t)} style={{ padding:'6px 14px', borderRadius:'8px 8px 0 0', fontSize:12, fontWeight:500, cursor:'pointer', border:'0.5px solid', borderBottom:'none', background:tab===t?'var(--bg)':'transparent', borderColor:tab===t?'var(--border)':'transparent', color:tab===t?'var(--text)':'var(--text-3)' }}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>)}
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px', background:'var(--bg)', borderTop:'0.5px solid var(--border)' }}>
        {tab==='basics'&&<>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
            ...
            <div style={{ gridColumn:'1/-1' }}><Input label="Full address" value={p.address} onChange={v=>set('address',v)} placeholder="Full address including postcode"/></div>
          </div>
          ...
          {p.currentValue&&p.purchasePrice&&<div style={{ fontSize:12, color:'var(--green)', marginBottom:14 }}>
            Estimated gain: {fmt(Number(p.currentValue)-Number(p.purchasePrice))}
          </div>}
          <Input label="Notes" value={p.notes} onChange={v=>set('notes',v)} placeholder="Anything to remember about this property"/>
        </>}
        ...
        {tab==='finance'&&<>
          ...
          {(p.rent||p.monthlyPayment)&&<div style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:12, padding:14, marginTop:4 }}>
            ...
            {p.rent&&p.monthlyPayment&&p.currentValue&&<Row label="Net yield" value={`${(((Number(p.rent)-Number(p.monthlyPayment))*12/Number(p.currentValue))*100).toFixed(1)}%`} valueColor="var(--green)"/>}
        ...
        {tab==='tenant'&&<>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
            ...
            <Input label="Tenancy end" value={p.tenancyEnd} onChange={v=>set('tenancyEnd',v)} placeholder="DD/MM/YYYY (if fixed term)"/>
            <Input label="Deposit amount (£)" value={p.depositAmount} onChange={v=>set('depositAmount',v)} placeholder="e.g. 850" type="number"/>
            <Select label="Deposit scheme" value={p.depositScheme} onChange={v=>set('depositScheme',v)} options={['','DPS Custodial','DPS Insured','TDS Custodial','TDS Insured','MyDeposits']}/>
          </div>
          <div style={{ background:'#fff8e1', border:'0.5px solid #EF9F27', borderRadius:9, padding:'10px 13px', fontSize:12, color:'#633806', lineHeight:1.6 }}>
            <strong>Reminder:</strong> Deposit must be protected within 30 days and prescribed information served on the tenant. Failure risks a penalty of 1–3× the deposit.
          </div>
        </>}

        {tab==='compliance'&&<>
          <div style={{ fontSize:12, fontWeight:500, color:'var(--text)', marginBottom:12 }}>Safety certificates</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
          ...
          <div style={{ fontSize:12, fontWeight:500, color:'var(--text)', margin:'8px 0 12px' }}>EPC</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
      ...
      <div style={{ padding:'14px 20px', background:'var(--surface)', borderTop:'0.5px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
        <div>
          {p.id&&<button onClick={()=>{if(confirm('Delete this property?')){onDelete(p.id);onClose()}}} style={{ fontSize:12, color:'var(--red)', background:'none', border:'none', cursor:'pointer' }}>Delete property</button>}
        </div>
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button onClick={handleSave} style={{ background:'var(--brand)', color:'#fff', border:'none', borderRadius:8, padding:'8px 24px', fontSize:13, fontWeight:500, cursor:'pointer' }}>Save property</button>
        </div>
      </div>
    </div>
  </div>
...
/* ─── Overview ─────────────────────────────────────────── */
function Overview({ portfolio, onAddDocs, user, onEditProperty }) {
  ...
  const netYield=totalValue>0&&net>0?((net*12/totalValue)*100).toFixed(1):null
  ...
  async function sendReminders(){
    setSending(true)
    try{
      await fetch('/api/reminders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userEmail:user.emailAddresses?.[0]?.emailAddress,userName:user.firstName,portfolio})})
  ...
  return <div className="fade-up">
    {urgent.length>0&&<div style={{ background:'#fce8e6', border:'0.5px solid #E24B4A', borderRadius:12, padding:'12px 14px', marginBottom:14, color:'#791F1F', fontSize:12, lineHeight:1.8 }}>
    ...
    {props.length===0?<DropZone onFiles={onAddDocs}/>:<>
      <div style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:14, padding:16, marginBottom:12 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          ...
          <button onClick={sendReminders} disabled={sending} style={{ background:emailSent?'var(--green-bg)':'var(--brand-light)', color:emailSent?'var(--green)':'var(--brand)', border:'none', borderRadius:7, padding:'5px 12px', fontSize:11, fontWeight:500, cursor:'pointer' }}>
            {emailSent?'✓ Sent!':sending?'Sending…':'📧 Email reminders'}
          </button>
        </div>
        {upcoming.length===0&&urgent.length===0
          ?<div style={{ fontSize:12, color:'var(--text-3)', padding:'6px 0' }}>No upcoming compliance issues.</div>
          :upcoming.map((t,i)=><div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:i<upcoming.length-1?'0.5px solid var(--border)':'none', gap:8, flexWrap:'wrap' }}>
...
function Properties({ portfolio, onAddDocs, onEdit, onAdd }) {
  ...
  return <div className="fade-up">
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <button onClick={onAdd} style={{ background:'var(--brand)', color:'#fff', border:'none', borderRadius:8, padding:'7px 16px', fontSize:12, fontWeight:500, cursor:'pointer' }}>+ Add property</button>
      </div>
    </div>
    ...
    {props.length===0?<DropZone onFiles={onAddDocs}/>:props.map(p=>{
      ...
      return <div key={p.id} style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:14, padding:18, marginBottom:14 }}>
        ...
        {(equity||grossYield||ltv)&&<div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:16 }}>
          {equity&&<div style={{ background:'var(--brand-subtle)', borderRadius:9, padding:'10px 12px' }}><div style={{ fontSize:10, color:'var(--brand)', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:4 }}>Equity</div><div style={{ fontSize:15, fontWeight:600, fontFamily:'var(--mono)', color:'var(--brand)' }}>{fmt(equity)}</div></div>}
          {ltv&&<div style={{ background:'var(--surface2)', borderRadius:9, padding:'10px 12px' }}><div style={{ fontSize:10, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:4 }}>LTV</div><div style={{ fontSize:15, fontWeight:600, fontFamily:'var(--mono)' }}>{ltv}%</div></div>}
          {grossYield&&<div style={{ background:'var(--surface2)', borderRadius:9, padding:'10px 12px' }}><div style={{ fontSize:10, color:'var(--text-3)', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:4 }}>Gross yield</div><div style={{ fontSize:15, fontWeight:600, fontFamily:'var(--mono)' }}>{grossYield}%</div></div>}
          {netYield&&<div style={{ background:'var(--green-bg)', borderRadius:9, padding:'10px 12px' }}><div style={{ fontSize:10, color:'var(--green)', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:4 }}>Net yield</div><div style={{ fontSize:15, fontWeight:600, fontFamily:'var(--mono)', color:'var(--green)' }}>{netYield}%</div></div>}
        </div>}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
          <div>
            ...
            {p.tenancyEnd    &&<Row label="End"         value={p.tenancyEnd}/>}
            {p.depositAmount &&<Row label="Deposit"     value={fmt(Number(p.depositAmount))}/>}
            {p.depositScheme &&<Row label="Scheme"      value={p.depositScheme}/>}
            {p.lender        &&<Row label="Lender"      value={p.lender}/>}
            {p.mortgage      &&<Row label="Mortgage"    value={fmt(Number(p.mortgage))}/>}
            {p.rate          &&<Row label="Rate"        value={`${p.rate}%`}/>}
            {p.fixedEnd      &&<Row label="Fixed until" value={p.fixedEnd}/>}
            {p.monthlyPayment&&<Row label="Monthly pmt" value={fmt(Number(p.monthlyPayment))}/>}
            {p.ercRate       &&<Row label="ERC"         value={`${p.ercRate}%`}/>}
          </div>
          <div>
  ...
  return <div className="fade-up">
    ...
    <div style={{ background:'var(--surface)', border:'0.5px solid var(--border)', borderRadius:16, padding:20 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:8 }}>
        <div>
          <div style={{ fontFamily:'var(--display)', fontSize:22, fontWeight:400, marginBottom:4 }}>{leg.name}</div>
          <div style={{ fontSize:12, color:'var(--text-3)' }}>{leg.status} · {leg.effectDate}</div>
        </div>
        <Pill type={leg.impact==='critical'?'red':leg.impact==='high'?'amber':'grey'}>{leg.impact==='critical'?'Critical':leg.impact==='high'?'High impact':'Ongoing'}</Pill>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        <div>
          <div style={{ fontSize:12, fontWeight:600, marginBottom:10, display:'flex', alignItems:'center', gap:6 }}><span style={{ width:8, height:8, borderRadius:'50%', background:'var(--green)', display:'inline-block' }}/>Current law</div>
          {leg.current.map((c,i)=><div key={i} style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.65, padding:'7px 0', borderBottom:i<leg.current.length-1?'0.5px solid var(--border)':'none' }}>{c.text}</div>)}
        </div>
        <div>
          <div style={{ fontSize:12, fontWeight:600, marginBottom:10, display:'flex', alignItems:'center', gap:6 }}><span style={{ width:8, height:8, borderRadius:'50%', background:'var(--amber)', display:'inline-block' }}/>Upcoming changes</div>
          {leg.upcoming.map((u,i)=><div key={i} style={{ fontSize:12, lineHeight:1.65, padding:'7px 0', borderBottom:i<leg.upcoming.length-1?'0.5px solid var(--border)':'none', color:u.severity==='red'?'#791F1F':u.severity==='green'?'var(--green)':'#633806' }}>{u.severity==='red'?'⚠️ ':u.severity==='green'?'✓ ':'· '}{u.text}</div>)}
        </div>
      </div>
      <div style={{ background:'var(--brand-subtle)', border:'0.5px solid rgba(27,94,59,0.15)', borderRadius:10, padding:'14px 16px' }}>
        ...
        {leg.actions.map((a,i)=><div key={i} style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.65, padding:'5px 0', borderBottom:i<leg.actions.length-1?'0.5px solid rgba(27,94,59,0.1)':'none', display:'flex', gap:8 }}><span style={{ color:'var(--brand)', fontWeight:600, flexShrink:0 }}>{i+1}.</span>{a}</div>)}
      </div>
    </div>
  </div>
}
...
function AITab({ portfolio }) {
  ...
  const [messages,setMessages]=useState([{role:'assistant',content:n>0?`I can see your portfolio of ${n} propert${n===1?'y':'ies'}. Ask me anything about compliance, EPC, finances, or the Renters' Rights Bill.`:`Welcome to Lettly AI. Add properties or drop documents first, then I can give specific advice about your portfolio.`}])
  const [input,setInput]=useState(''),[loading,setLoading]=useState(false)
  const scrollRef=useRef(null)
  useEffect(()=>{if(scrollRef.current)scrollRef.current.scrollTop=scrollRef.current.scrollHeight},[messages])
  const quickQ=n>0?['What are my most urgent compliance actions?',"How does the Renters' Rights Bill affect me?",'Which properties have EPC risk?','Should I move to a limited company?']:["What does the Renters' Rights Bill mean?",'What compliance certificates do I need?','What is EPC minimum C?','What is Section 24 tax?']
  ...
  return <div className="fade-up">
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'min(500px,60vh)' }}>
      <div ref={scrollRef} style={{ flex:1, overflowY:'auto', padding:12, display:'flex', flexDirection:'column', gap:10, background:'var(--surface2)', borderRadius:'12px 12px 0 0', border:'0.5px solid var(--border)', borderBottom:'none' }}>
        {messages.map((m,i)=><div key={i} style={{ maxWidth:'88%', alignSelf:m.role==='user'?'flex-end':'flex-start' }}>
...
/* ─── Root ─────────────────────────────────────────────── */
const TABS=[{id:'overview',label:'Overview'},{id:'properties',label:'Properties'},{id:'legislation',label:'Legislation'},{id:'ai',label:'Lettly AI'}]
...
export default function Dashboard() {
  ...
  useEffect(()=>{
    if(!user?.id)return
    getPortfolio(user.id).then(data=>{setPortfolio(data||{properties:[]});setLoaded(true)})
  },[user?.id])
  ...
  return <>
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}
      ...
      onDrop={e=>{e.preventDefault();const f=Array.from(e.dataTransfer.files).filter(f=>f.type==='application/pdf'||f.type.startsWith('image/'));if(f.length)handleFiles(f)}}>
      ...
      <nav style={{ background:'var(--surface)', borderBottom:'0.5px solid var(--border)', padding:'0 16px', display:'flex', alignItems:'center', justifyContent:'space-between', height:54, position:'sticky', top:0, zIndex:100, gap:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          <div style={{ width:30, height:30, background:'var(--brand)', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ color:'#fff', fontSize:14, fontWeight:700, fontFamily:'var(--display)', fontStyle:'italic' }}>L</span></div>
          <span style={{ fontFamily:'var(--display)', fontSize:17, fontWeight:400 }}>Lettly</span>
        </div>
        <div style={{ display:'flex', gap:1, background:'var(--surface2)', padding:3, borderRadius:9, overflow:'hidden' }}>
          {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{ background:tab===t.id?'var(--surface)':'transparent', border:tab===t.id?'0.5px solid var(--border)':'none', padding:'5px 11px', borderRadius:7, fontFamily:'var(--font)', fontSize:12, color:tab===t.id?'var(--text)':'var(--text-2)', fontWeight:tab===t.id?500:400, cursor:'pointer', whiteSpace:'nowrap' }}>
            {t.label}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          <button onClick={()=>setShowDrop(v=>!v)} style={{ background:'none', border:'0.5px solid var(--border-strong)', borderRadius:7, padding:'6px 10px', fontSize:12, color:'var(--text-2)', cursor:'pointer', display:'flex', alignItems:'center', gap:5, whiteSpace:'nowrap' }}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add docs
          </button>
      ...
      <div className="dash-content">
        {tab==='overview'&&<div style={{ marginBottom:18 }}>
          <h1 style={{ fontFamily:'var(--display)', fontSize:'clamp(22px,4vw,28px)', fontWeight:300, marginBottom:3 }}>Good {getGreeting()}, {user?.firstName||'there'}</h1>
          <p style={{ fontSize:13, color:'var(--text-3)' }}>{(portfolio.properties||[]).length===0?'Add a property manually or drop documents to get started.':`${(portfolio.properties||[]).length} propert${(portfolio.properties||[]).length===1?'y':'ies'} · saved to your account`}</p>
        </div>}
        {tab==='overview'    &&<Overview    portfolio={portfolio} onAddDocs={handleFiles} user={user} onEditProperty={p=>{setFormProp(p);setTab('properties')}}/>}
